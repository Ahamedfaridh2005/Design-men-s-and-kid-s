...
export default function AdminCustomers() {
  ...
  const adminCount = customers.filter(c => c.email?.includes('230055.cs') || c.email?.includes('230222.cs')).length || 2; 
      ...
      <div className="max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="border border-border p-6 bg-transparent">
            ...
            <p className="font-heading text-2xl mb-1">{customers.length || 2}</p>
          </div>
          <div className="border border-border p-6 bg-transparent">
          ...
          <table className="w-full text-sm text-left font-body min-w-[700px]">
            ...
            <tbody>
              {loading ? (
                ...
                const displayName = customer.full_name || customer.email?.split('@')[0] || "Guest";
                const initial = displayName.charAt(0).toUpperCase();
                const role = customer.email?.includes('230055.cs') || customer.email?.includes('230222.cs') ? 'ADMIN' : 'USER';
                const roleColor = role === 'ADMIN' ? 'bg-[#1a1a1a] text-white' : 'bg-[#e5e5e5] text-[#888888]';
                
                return (
                <tr key={customer.id} className="border-b border-border last:border-0 hover:bg-[#fbfaf8]">
